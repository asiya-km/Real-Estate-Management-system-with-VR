...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_bank_transfer'])) {
    ...
    if (empty($transaction_id)) {
        $_SESSION['error'] = "Transaction reference number is required";
    ...
    if (isset($_FILES['receipt']) && $_FILES['receipt']['error'] == 0) {
        ...
        if (!in_array(strtolower($ext), $allowed)) {
            $_SESSION['error'] = "Invalid file format. Only PDF, JPG, JPEG, and PNG are allowed.";
            header("Location: payment.php?booking_id=" . $booking_id);
            exit();
        }
        
        $new_filename = 'RECEIPT_' . $booking_id . '_' . time() . '.' . $ext;
        ...
        if (move_uploaded_file($_FILES['receipt']['tmp_name'], $upload_dir . $new_filename)) {
        ...
        $_SESSION['error'] = "Receipt file is required";
        header("Location: payment.php?booking_id=" . $booking_id);
        exit();
    }
    ...
    mysqli_stmt_bind_param($stmt, 'dssdi', $deposit_amount, $transaction_id, $receipt_file, $remaining_balance, $booking_id);
    
    if (mysqli_stmt_execute($stmt)) {
        // Set success message
        $_SESSION['success_message'] = "Your payment has been submitted for verification. We will review it shortly and update your booking status.";
