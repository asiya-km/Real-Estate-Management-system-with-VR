...
// Get booking ID and phone number
...
if (empty($transaction_id)) {
    $_SESSION['error'] = "Please provide the Telebirr transaction ID.";
    header("Location: payment.php?booking_id=" . $booking_id);
    exit();
}
...
$reference = 'TLB' . date('Ymd') . str_pad($booking_id, 6, '0', STR_PAD_LEFT);

$update_stmt = mysqli_prepare($con, $update_query);
mysqli_stmt_bind_param($update_stmt, 'ssdsii', $phone, $reference, $amount, $transaction_id, $booking_id, $user_id);
$update_result = mysqli_stmt_execute($update_stmt);

if ($update_result) {
    // Send notification to admin (optional)
    $admin_email = "admin@example.com"; // Replace with actual admin email
    $subject = "New Telebirr Payment Pending Verification";
    $message = "A new Telebirr payment is pending verification:\n\n";
    $message .= "Booking ID: #" . str_pad($booking_id, 6, '0', STR_PAD_LEFT) . "\n";
    $message .= "Property: " . $booking['title'] . "\n";
    $message .= "Amount: ETB " . number_format($amount, 2) . "\n";
    $message .= "Customer Phone: " . $phone . "\n";
    $message .= "Transaction ID: " . $transaction_id . "\n";
    $message .= "Reference: " . $reference . "\n";
    $message .= "Date: " . date('Y-m-d H:i:s') . "\n\n";
    $message .= "Please verify this payment in your Telebirr account.";
    
    mail($admin_email, $subject, $message);
    
    // Set success message and redirect to my_payments.php
    $_SESSION['success'] = "Your Telebirr payment is pending verification. We will confirm your payment shortly.";
